...
___Поддерживается запуск SQLite внутри запущенного контейнера. Если имя таблицы и путь не изменялись, то запуск бинарника сразу откроет базу данных "scheduler.db". И можно работать с таблицей.___
